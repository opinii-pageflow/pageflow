  ...
  useEffect(() => {
    ...
    async function loadAdminData(retryCount = 0) {
      ...
      try {
        ...
        const [allClients, profilesData, allRequests] = await Promise.all([
        ...
        setTotalProfiles(profilesData?.length || 0);

        setAllProfiles((profilesData || []).map((p: Profile) => ({
          ...p,
                  ...
                  <div key={profile.id} className="flex items-center justify-between gap-3 px-6 py-3.5 hover:bg-white/[0.02] transition-colors group">
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <a href={`#/u/${profile.slug}`} target="_blank" rel="noreferrer" className="text-[10px] text-zinc-600 truncate hover:text-white">/{profile.slug}</a>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      <button
                        ...
                        className={clsx(
                          ...
                          profile.featured ? "bg-amber-500/20 border-amber-500/30" : "bg-zinc-800 border-white/5"
                        )}
                      >
                        <div className={clsx("w-4 h-4 rounded-full transition-all duration-300", profile.featured ? "translate-x-5 bg-amber-400" : "translate-x-0 bg-zinc-600")} />
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden">
              ...
              <div className="divide-y divide-white/[0.03]">
                {upgradeRequests.map((request) => (
                  <div key={request.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                          <span className="text-[11px] text-zinc-500"><Mail size={10} className="inline mr-1" /> {request.email}</span>
                          <span className="text-[11px] text-emerald-500 font-bold"><MessageCircle size={10} className="inline mr-1" /> {request.whatsapp}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        disabled={updatingRequestId === request.id || request.status === 'closed'}
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-2xl p-6">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-9 h-9 bg-white/5 rounded-xl flex items-center justify-center text-orange-500">
                  <Cpu size={18} />
                </div>
                <h4 className="text-base font-black tracking-tight">Host Status</h4>
              </div>
              <div className="space-y-3 divide-y divide-white/5">
                <div className="flex justify-between py-3">
                  <span className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Version</span>
