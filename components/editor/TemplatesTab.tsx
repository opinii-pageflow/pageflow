...
const TemplatesTab: React.FC<Props> = ({ profile, onUpdate }) => {
  ...
  const templates: Tpl[] = [
    // Essenciais
    { id: 'Minimal Card', label: 'Minimal', icon: <Layout size={20} /> },
    { id: 'Button List Bold', label: 'Bold List', icon: <Rows3 size={20} /> },
    { id: 'Avatar Left', label: 'Left Side', icon: <PanelLeft size={20} /> },
    { id: 'Corporate', label: 'Corporate', icon: <Briefcase size={20} /> },
    { id: 'Button Grid', label: 'Simple Grid', icon: <Grid3X3 size={20} /> },
    { id: 'Light Clean', label: 'Light', icon: <Palette size={20} /> },
    ...
    { id: 'Neon', label: 'Neon Glow', icon: <Zap size={20} /> },
    ...
    { id: 'Split Header', label: 'Split', icon: <Grip size={20} /> },
  ];
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-1 duration-300">
      <header className="flex items-center justify-between px-1">
        <div>
          <h3 className="text-xl font-bold tracking-tight">Layouts</h3>
          <p className="text-xs text-zinc-500">Escolha a estrutura do seu perfil.</p>
        </div>
      </header>
      ...
      <div className="p-4 bg-zinc-900/40 border border-white/5 rounded-2xl flex items-center gap-4">
        ...
        <p className="text-[10px] text-zinc-500 font-bold leading-relaxed uppercase tracking-wider">
          Templates com <Lock size={10} className="inline mx-1" /> estão disponíveis apenas nos planos <b>Pro ou superior</b>.
        </p>
      </div>
    </div>
  );
