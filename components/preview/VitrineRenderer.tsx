import {
    ...
    Instagram, Linkedin, Globe, Phone, Mail, MapPin, Youtube, Github, Facebook, Twitter, Music2, Send, AtSign, Tv, MessageSquare
} from 'lucide-react';
...
const VitrineRenderer: React.FC<Props> = ({ profile, showcase, isPreview }) => {
    ...
    const handleCTA = (item: ShowcaseItem) => {
        ...
        if (item.ctaType === 'whatsapp') {
            const msg = encodeURIComponent(item.ctaValue || `Olá! Tenho interesse no item: ${item.title}`);
            const phone = profile?.bookingWhatsapp?.replace(/\D/g, '') || '';
            window.open(`https://wa.me/${phone}?text=${msg}`, '_blank');
        } else if (item.ctaValue) {
            window.open(item.ctaValue, '_blank');
        }
    };
                                    ...
                                    <div className={clsx(
                                        ...
                                        <div className="flex-1">
                                            <div className={clsx(
                                                ...
                                                {item.basePrice > 0 && (
                                                    <div className={clsx(
                                                        "font-black text-emerald-400 tracking-tighter shrink-0",
                                            ...
                                            <p className={clsx(
                                                "text-zinc-500 font-medium leading-relaxed italic",
                                                showcase.itemTemplate === 'premium' ? "text-xs" : "text-[11px] line-clamp-2"
                                            )}
                                                style={{ fontFamily: bodyFontStack }}
                                            >
                                                {item.description}
                                        ...
                                        {item.options.length > 0 && (
                                            <div className="flex flex-wrap gap-2">
                                                {item.options.map((opt) => (
                                                    <div key={opt.id} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 text-[9px] font-bold text-zinc-400"
                                                        style={{ fontFamily: bodyFontStack }}
                                                    >
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
                    ...
                    <div className="relative w-full max-w-5xl max-h-[90vh] bg-zinc-950 border border-white/10 rounded-[3rem] overflow-hidden flex flex-col md:flex-row shadow-[0_0_100px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-500">
                        ...
                        <div className="md:w-1/2 p-8 sm:p-12 overflow-y-auto space-y-10 custom-scrollbar">
                            ...
                            <p className="text-zinc-500 text-sm sm:text-base leading-relaxed font-medium italic">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {selectedItem.options.map((opt) => (
                                            <div key={opt.id} className="flex justify-between items-center p-4 rounded-2xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-all">
